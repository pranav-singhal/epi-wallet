      ...
      Web3.checkIfMined(message.hash)
        .then((isMined) => {
          ...
          antdMessage.success({
            content: (
              <span>
                Transaction to send <b>{message.amount}</b> ETH to{" "}
                <b>{toTitleCase(message.to)}</b> has been confirmed.
              </span>
            ),
    ...
    <div className="crypto-message">
      <div
        className={classNames(`crypto-message-card`, {
          ...
          "crypto-message-card__declined": messageStatus === "declined",
        })}
      >
        ...
        <div className="crypto-message-card__amount">
          ...
          {messageStatus === "unconfirmed" &&
            message.type === MESSAGETYPES.RECIEVED && (
              <Button
                type="primary"
                shape="round"
